...
-(void)startPlayer:(NSString*)uri {
    ...
    moviePlayer.view.frame = self.viewController.view.bounds;
    ...
    [view insertSubview:activityIndicator aboveSubview:moviePlayer.view];
...
-(void) moviePlayerLoadStateDidChange:(NSNotification*)notification {
    NSLog(@"loadState: %lu", (unsigned long)moviePlayer.loadState);
    if (moviePlayer.loadState & MPMovieLoadStateStalled) {
        [activityIndicator startAnimating];
        [moviePlayer pause];
    } else if (moviePlayer.loadState & MPMovieLoadStatePlaythroughOK) {
        //if (!hasAddedMoviePlayerView){
            hasAddedMoviePlayerView = true;
    ...
    [message setObject:[NSNumber numberWithInteger:moviePlayer.loadState] forKey:@"loadState"];
